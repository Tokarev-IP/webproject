...
// lib/auth/shared/types.schema.json
...
var isValidBodyParams = ajv.compile(
  types_schema_default.definitions["ConfirmSignUpBody"] || {}
);
var client = new import_client_cognito_identity_provider.CognitoIdentityProviderClient({ region: process.env.REGION });
var handler = async (event) => {
  try {
    ...
    if (!isValidBodyParams(body)) {
      return {
        ...
        body: JSON.stringify({
          ...
          schema: types_schema_default.definitions["ConfirmSignUpBody"]
        })
      };
    }
    ...
    const command = new import_client_cognito_identity_provider.ConfirmSignUpCommand(params);
